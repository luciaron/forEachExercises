...
function doubleValues(arr){
    const resultArr  = arr.map(function(el) {
        return el * 2;
    })
    return resultArr;
}
...
function onlyEvenValues(arr){
    const resultArr = arr.filter(function(el) {
        return el % 2 === 0;
    })
    return resultArr;
}
...
function showFirstAndLast(arr){
    const resultArr = arr.map(function(el, i) {
        let first = arr[i][0];
...
function vowelCount(str){
    ...
    tempArr.forEach(function(letter) {
        if (vowels.indexOf(letter) !== -1) {            //uses only vowels
            if (resultObj[letter]) {
                resultObj[letter]++;
            } else {
                resultObj[letter] = 1;
...
function extractKey(arr, key){
    const resultArr = arr.map(function(obj, i) {
        return obj[key];
    });
    return resultArr;
}
...
function filterByValue(arr, key) {
    let resultArr = arr.filter(function(obj, i) {
        if (obj[key]){
            return obj;
        }
    });
...
function removeVowels(str) {
    ...
    const noVowelsArr = stringArr.filter(function(el) {
        return vowels.indexOf(el) === -1;
    });
    const resultStr = noVowelsArr.join('');
    return resultStr;
}
